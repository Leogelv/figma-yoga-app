...
- 🟢 Создание единого компонента PracticeScreen для отображения всех типов практик
  - 🟢 Режим видеоплеера для медитативных практик (с Kinescope)
  - 🟢 Режим таймера для телесных и дыхательных практик
...
## Предыдущие задачи: Реорганизация структуры папок и исправление ошибок сборки
...
- 🟡 Доработка медитационного квиз-флоу 
  ...
  - 🟡 Необходимо тестирование квиз-флоу после реорганизации компонентов
...
### Реорганизация структуры приложения для улучшения архитектуры и поддерживаемости
- 🟡 Создание логичной структуры компонентов
  - 🟢 Разделение UI-компонентов и компонентов домена (квиз, практика)
  - 🟢 Создание компонента TimerScreen в правильном месте 
  - 🟡 Обновление всех зависимых компонентов и импортов
...
### Тестирование после реорганизации компонентов
- 🔴 Проверка корректности работы Button компонента
  - [ ] Кнопка правильно отображается в разных вариантах (primary, secondary, etc.)
  - [ ] Пропс size работает как ожидается
  - [ ] Добавление новых вариаций не сломало старые
- 🔴 Проверка работоспособности Spinner компонента
  - [ ] Компонент отображается корректно на разных экранах
- 🔴 Проверка работы таймера в новой локации
  - [ ] Компонент TimerScreen правильно импортируется и используется
  - [ ] Функционал таймера не изменился после переноса
